...
import { SandboxPolicySchema } from "../types.js";
...
export async function handler(
  ...
  try {
    const response = await client.updateConfig({ name, policy, global: false });
    return jsonResult(response);
  } catch (err) {
    return textError(formatGrpcError("policy-set-failed", err));
  }
}
