...
| `@mcp-conductor/server`             | CLI binary `conductor`. Loads `conductor.json`, builds providers via `provider-factory.ts`, wires the gateway. `src/main.ts` is the programmatic entry; `src/cli.ts` is the executable.                                                                                                                          |
