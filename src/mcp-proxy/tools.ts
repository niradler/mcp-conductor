  ...
  return {
    list_mcp_servers: {
      tool: {
        ...
        description:
          'List all available MCP servers with their tools, resources, and prompts. Use this to discover what MCP servers are available.',
        inputSchema: {},
      },
    ...
    get_tool_details: {
      tool: {
        ...
        description:
          'Get detailed information about specific tools from an MCP server, including full schemas and descriptions.',
        inputSchema: {
      ...
      handler: async (
        { server, tools }: { server: string; tools?: string[] },
      ): Promise<{ tools: MCPToolDetails[] }> => {
        await manager.reloadIfNeeded()
